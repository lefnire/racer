      ...
      store.eachContext(function (context) {
        ...
        context.guardReadPath = function (req, res, next) {
          context.guardReadPath.add( function (req, res, next) {
            if (! store.accessControl.readPath) return next();
            if (! req.didMatchAGuard)
              return res.fail(new Error('403 Unauthorized: No access control declared for path ' + req.target));
            next();
          });
        ...
        context.guardQuery = function (req, res, next) {
          context.guardQuery.add( function (req, res, next) {
            if (! store.accessControl.query) return next();
            ...
            for (var motifName in motifs) {
              if (! matchingGuardFor || !matchingGuardFor[motifName])
                return res.fail(new Error('403 Unauthorized: No access control declared for motif ' + motifName));
            }
            next();
          });
        ...
        context.guardWrite = function (req, res, next) {
          if (! store.accessControl.write) return next();
          context.guardWrite.add( function (req, res, next) {
            if (! req.didMatchAGuard) {
              var txn = req.data;
              return res.fail(new Error('403 Unauthorized: No access control declared for mutator: ' +
...
function createPathGuard (pattern, callback) {
  ...
  function guard (req, res, next) {
    ...
    captures.push(function (isAllowed) {
      if (!isAllowed) return res.fail(new Error('403 Unauthorized'));
      return next();
    }, res.fail);

    callback.apply(caller, captures);
  }
...
function createQueryGuard (ns, motif, callback) {
   ...
   * Otherwise res.fail('403 Unauthorized');
   */
  function guard (req, res, next) {
    var queryTuple = req.target
      , queryNs = queryTuple[0]
...
function createWriteGuard (mutator, target, callback) {
  ...
  function guard (req, res, next) {
    ...
    captures.push.apply(captures, args);
    captures.push(function (isAllowed) {
      if (!isAllowed) return res.fail(new Error('403 Unauthorized'), txn);
      return next();
    }, res.fail);
