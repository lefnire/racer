...
, events: {
  ...
  , init: function (model) {
      ...
      model._removeTxn = function (txnId) {
        ...
        var i = model._txnQueue.indexOf(txnId);
        if (~i) {
  ...
  , socket: function (model, socket) {
      ...
      function commit (txn) {
        ...
        socket.emit('txn', txn, model._startId, txn.dontPersist);
      }
    }
  }
  ...
  , _specModel: function () {
      ...
      while (i < len) {
        ...
        if (transaction.isCompound(txn)) {
          ...
          if (docIsntPresent(this, txn)) continue;
