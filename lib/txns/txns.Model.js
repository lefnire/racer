...
function createSocketHandlers(model) {
  var handlers = {}
  ...
  handlers.replace = function (data, num) {
  ...
  handlers.newTxns = function (newTxns, num) {
  ...
  , onTimeout: function () {
    ...
    console.warn("Client timed out; asking for snapshot!");
    // TODO Don't do this if we are also responding to a resyncWithStore
    model.socket.emit('fetch:snapshot', memory.version + 1, model._startId, model._subs());
  }
  });
  ...
  handlers.disconnect = teardownResendInterval;
  ...
  handlers.txnOk = function (rcvTxn, num) {
  ...
  handlers.txnErr = function (err, txnId) {
  ...
  function commit (txn) {
    ...
    // txn itself is an array, and txn.dontPersist is a property on that array. Socket.io will remove array properties
    // and just pass up the txn array, so we send txn.dontPersist as a 3rd argument
      model.socket.emit('txn', txn, model._startId, txn.dontPersist);
  }
...
, events: {
  ...
  , socket: function (model, socket) {
    var handlers = model._socketHandlers;
    if (!handlers)
      handlers = model._socketHandlers = createSocketHandlers(model);
    socket.on('disconnect', handlers.disconnect);
    socket.on('txn', handlers.txn);
    socket.on('txnOk', handlers.txnOk);
    socket.on('txnErr', handlers.txnErr);
    socket.on('snapshotUpdate:newTxns', handlers.newTxns);
    socket.on('snapshotUpdate:replace', handlers.replace);
  }
  }
...
, proto: {
  ...
  , _sendOp: function (method, args, cb) {
      ...
      if (~ path.indexOf('.null')) {
        return console.warn('You were about to set on a path including null: ' + path);
      }
      ...
      // If we want to prevent persistence (we're queueing a batch operation), we can set model._dontPersist
      // and we will propagate that via txn
      txn.dontPersist = this._dontPersist;
//      txn.isPrivate = this._dontPersist
