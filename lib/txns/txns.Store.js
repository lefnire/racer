...
, events: {
    init: function (store) {
      ...
      store._pubSub.on('txn', function (clientId, txn) {
        ...
        if (socket) {
          ...
          if (ver === -1) { //If the txn is from an external source, apply it immediately
            socket.emit('txn', txn);
          } else if (ver > socket.__ver) {
            socket.__ver = ver;
            store.serialEmit(socket, 'txn', txn);
            store.serialCleanup(txn); // Flush the txn buffer
          }
          return;
        }
        ...
        store._txnBuffers.send(clientId, txn);
      });
    }
  ...
  , middleware: function (store, middleware) {
      ...
      function writeToDb (req, res, next) {
        ...
        // work-around method for lack of model.update() function - don't write these changes, as
        // we'll be manually send up a batch update operation
        if (!!txn.dontPersist) return next()
  ...
  , socket: function (store, socket, clientId) {
      ...
      socket.on('txn', function (txn, clientStartId, dontPersist) {
        // txn is an array, and though we previously set txn.dontPersist as a property, socket.io only sends the array
        // So we send txn.dontPersist as a 3rd argument and put it back onto the object
        txn.dontPersist = dontPersist;
