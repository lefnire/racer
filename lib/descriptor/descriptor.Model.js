...
, proto: {
  ...
  , _upstreamData: function (descriptors, cb) {
      if (!this.connected) return cb && cb('disconnected');
  ...
  , _subs: function () {
      ...
      types.each( function (name, type) {
        subs.push.apply(subs, type.subs(model));
      });
      return subs;
    }
  ...
  , _allSubs: function () {
      var subs = []
        ...
        , model = this;
      types.each( function (name, type) {
        subs.push.apply(subs, type.allSubs(model));
      });
      return subs;
    }
  ...
  , _addData: function (data) {
      ...
      var wasSilent = this._silent;
      data = data.data;

      // If there are fewer than 10 new docs, raise normal change events.
      var useBulk = data.length > 10;
      ...
      if (!useBulk) return;
      // Prevent bulkUpdate handlers from emitting set events too
      // Anything that cares about changes must handle bulkUpdate
      // to catch changes from the subscription. Thus, I ensure
      // sure that they don't see extra change events emitted
      // by other bulkUpdate handlers before they run.
      this._silent = true;
      if (!wasSilent) {
...
function subscribe (model, descriptors, cb, always) {
  ...
  , scopedResult: function (scopedModel) {
    ...
    always: always
  , onRemoved: function (descriptor) { model._removeSub([descriptor]); }
  });
