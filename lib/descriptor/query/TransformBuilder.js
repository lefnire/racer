...
TransformBuilder.fromJson = function (model, source) {
  ...
  var customDeps= source.dependencies;
  ...
  // If the caller manually add dependencies on the server, put them in the client too.
  for (var i = 0; i < customDeps.length; i++) {
    if (builder.dependencies.indexOf(customDeps[i] === -1))
      builder.dependencies.push(customDeps[i]);
  }
