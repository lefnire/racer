var filter = require('../../computed/filter');  // This code runs before filter.js sets exports, due to mutually recursive require()s.
...
QueryBuilder.prototype.where = function (param) {
  ...
  for (var fieldName in param) {
    ...
    if (arg instanceof RegExp) {
      this.regexp(arg);
    } else if (arg === null || arg.constructor !== Object) {
      this.equals(arg);
...
QueryBuilder.hash = function (json, filterFn) {
  ...
  for (var method in json) {
    ...
    switch (method) {
      ...
      case 'regexp':
        for (var k in val) {
          if (val[k] instanceof RegExp)
            val[k] = filter.regexObject(val[k]);
          else if (val[k] instanceof Array)
            val[k] = val[k].sort();
        }
