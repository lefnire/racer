...
function scopedResult(model, queryTuple, params) {
  var memoryQuery = model.registeredMemoryQuery(queryTuple)
    , queryId = model.registeredQueryId(queryTuple);
  return setupQueryModelScope(
    model, memoryQuery, queryId, 
    function onRemoved(){
      model.unregisterQuery(queryTuple);
      if (params && params.onRemoved) params.onRemoved(queryTuple);
    }
  );
}
...
function loadQueries(bundle) {
  for (var i = 0, l = bundle.length; i < l; i++) {
    ...
    scopedResult(this, queryTuple, {
      onRemoved: function (descriptor) {
        this._removeSub([descriptor]);  // If the query self-destructs, unregister it from the server
      }.bind(this)
    });
  }
}
