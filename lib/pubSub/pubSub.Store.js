...
, events: {
    init: function (store, opts) {
      ...
      ['addDoc', 'rmDoc'].forEach( function (messageType) {
        pubSub.on(messageType, function (clientId, params) {
          ...
          store.serialEmit(socket, messageType, params);
  ...
  , socket: function (store, socket, clientId) {
      ...
      socket.on('subscribe', function (targets, contextName, cb) {
        ...
        var res = {
          fail: function (err) { cb(err.message || err); }  // Error instances don't serialize correctly
        , send: function (data) {
            cb(null, data);
          }
        };
  ...
  , middleware: function (store, middleware) {
      ...
      // Subscribe
      middleware.subscribe.add(function (req, res, next) {
        var pubSubTargets = compilePubSubTargets(store, req.targets);
...
, proto: {
  ...
  , unsubscribe: function (socket, targets, context, callback) {
      targets = compilePubSubTargets(this, targets);
  ...
  , publish: function (path, type, data, meta) {
      ...
      // Publish asynchronously, to guarantee that
      // the txn receiver won't run while applying
      // original transaction (which breaks flush)
...
function compilePubSubTargets (store, targets) {
