...
function filterFnFromQuery (json) {
  ...
  if (json) for (var method in json) {
    ...
    if (Array.isArray(pred)) predicates.push.apply(predicates, pred);
...
predicateBuilders.regexp = function (params) {
  ...
  for (var fieldName in params) {
    ...
    if (typeof val === 'string')  // Eagerly create regex objects for strings
      ...
      regexifiedParams = val;           // Leave scoped models untouched; I'll need to parse them every time
    else if (val instanceof RegExp) {   // Use existing regexes as-is, but convert them to objects for JSON
      regexifiedParams = val;
...
function toLikeRegexp(expr){
  return new RegExp("^" + util.escapeRegExp(expr).replace(/%/g, '.*') + "$", "i");
}
predicateBuilders.like = function (params) {
  // The params object is serialized to the
  // browser and can only contain JSON-able
  // primitives.  Therefore, I cannot store
  // regexes in the original object.
  var regexifiedParams = {};
  for (var fieldName in params) {
    if (typeof params[fieldName] === 'string')  // Eagerly create regex objects for strings
      regexifiedParams[fieldName] = toLikeRegexp(params[fieldName]);
    else
      regexifiedParams = params[fieldName]; // Leave scoped models untouched; I'll need to parse them every time
  }
  return createDocPredicates(regexifiedParams, function (fieldName, expr, doc) {
    if (expr && expr.$ref) {
      expr = toLikeRegexp(this.model.get(expr.$ref));
    }
    return expr.test(lookup(fieldName, doc, this.model));
  });
};
