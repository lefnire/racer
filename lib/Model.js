...
, _setSocket: function (socket) {
    ...
    function onFatalErr (reason) {
      ...
      if (socket.connected)   // If a fatal error happens during handshake, disconnect() won't work.
        socket.disconnect();
    }
...
, leaf: function (path) {
  ...
  // an object with a cleanup() method instead, since they generate their own
  // handler methods.
...
, once: function (type, pattern, callback) {
   ...
   * @return {Object}            An object with a cleanup() function that removes the handler.  These handlers cannot be passed to removeListener()
   */
...
function createPathListener(model, type) {
  ...
  model._on(type, function eventPathRaiser() {
    ...
    // The ping event raises (path, result).
