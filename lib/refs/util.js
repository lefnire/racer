 ...
 */
RefEmitter.prototype.onRefListMember = function (node, memberKeyPath, domainPath, id, rest, index) {
  ...
  if (method === 'set') {
    ...
    this.args[0] = joinPaths(domainPath, index === null ? id : index, rest);
  } else if (method === 'del') {
    id = node.id;
    if (id == null) {
...
RefEmitter.prototype.onRef = function (node, dereffedToPath, rest, hardLink, idMissing) {
  // Allow ref to be deleted or over-written if not a hardLink
  if (!hardLink && !rest.length && (this.method === 'del' || this.method == 'set')) return;
  if (idMissing)  // Suppress mutations on keyed refs that don't point to anything
    this.args[0] = false;
  else
