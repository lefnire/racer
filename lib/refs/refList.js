...
function createRefList (model, from, to, key, opts) {
  if (!from || (!to && to !== "") || !key) {
  ...
  if (opts && opts.dependencies === 'destroy') {
    model.on('remove', key, function (index, howMany, removedIds, isLocal) {
      if (! isLocal) return;
      var docs = model.get('to');
      for (var i = 0, l = removedIds.length; i < l; i++) {
        var id = removedIds[i];
        if (Array.isArray(docs)) {
          var index = indexOfFn(docs, function (doc) {
            return doc.id === is;
          });
          if (index !== -1) model.remove(to, index);
        } else {
          model.del(to + '.' + id);
        }
      }
...
function createGetter (from, to, key) {
  ...
  return function getterRefList (data, pathToRef, rest, meta) {
    ...
    if (!rest.length) {
      ...
      var resultPath, result, pathInDomain;
      ...
      if (meta.refEmitter) {
        meta.refEmitter.onRefListMember(result, joinPath(key, index), resultPath || to, id, rest, pathInDomain === void 0 ? null : pathInDomain);
      }
