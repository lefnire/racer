...
function createGetterWithKey (to, key, hardLink) {
  ...
  return function getterWithKey (data, pathToRef, rest, meta) {
    ...
    if (meta.refEmitter) {
      meta.refEmitter.onRef(node, path, rest, hardLink, id === null);
    }
...
function setupRefWithKeyListeners (model, from, to, key, getter) {
  ...
  refListener.add(to + '.*', function (path) {
    ...
    if (remainder.substring(0, index + 1) === keyPath + '.') {
      ...
      // to could be an Array, while key could be of an id that's a doc in the
      // array. e.g., to -> [{id: 'a'}, {id: 'b'}], key -> 'b'

      // In this case, find the matching index
      var arr = model.get(to);
      if (Array.isArray(arr)) {
        var index = indexOfFn(arr, function (doc) {
          return doc.id === keyPath;
        });
        remainder = remainder.substring(('' + index).length + 1, remainder.length);
        if (~index) return from + '.' + remainder;
      }
    }
