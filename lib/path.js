...
// Runs a callback on each parent path of a nested path.  (but not the full path itself)
exports.eachParent = function eachParent(path, callback) {
  var lastDot = path.length, index = 0;
  while (0 < (lastDot = path.lastIndexOf('.', lastDot - 1))) {
    callback(path.substring(0, lastDot), index++);
  }
};
