var EventEmitter = require('events').EventEmitter
  ...
  , merge = require('racer-util/object').merge
  ;
 ...
 * @return {Model}
 */
Store.prototype.createModel = function (opts) {
  opts = merge({
    store: this
  , _ioUri: this._ioUri
  ...
  var model = new this.racer.protected.Model(opts);
...
Store.prototype._sendToDb = function (method, args, done) {
  ...
  function next () {
    ...
    if (! handler) {
      console.log(new Error('No persistence handler for ' + method + '(' + args.join(',') + ')'));
      return done();
    }
    ...
    var captures = (path === '')       ? ['']
                 : (match.length > 1)  ? match.slice(1)
                                       : [match[0]];
    captures.push.apply(captures, rest);
    captures.push(lockingDone, next)
    return fn.apply(null, captures);
  }
