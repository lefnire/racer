...
describe 'access control', ->
  ['fetch', 'subscribe'].forEach (reader) ->
    ...
    describe "unconfigured Model #{reader} paths", ->
      describe 'asynchronous predicates', ->
        it 'should fail on the first page load', (done) ->
          run = setup
            config: (racer, store) ->
    ...
    describe "unconfigured Model #{reader} queries", ->
      describe 'asynchronous predicates', ->
        it "should fail when #{reader} originates on the server", (done) ->
          run = setup
            config: (racer, store) ->
  ...
  describe 'forbidden writes', ->
    it 'should fail when the transaction originates on the server', (done) ->
      run = setup
        config: (racer, store) ->
          store.accessControl.readPath = true
          store.accessControl.query = true
